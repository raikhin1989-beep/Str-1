...
#
# Считаем по адресу, потому что спрашивать про виски может кто угодно, без
# всякой ссылки. Но за одним роутером сидит вся компания, и на неё это один
# счётчик: было 20 в час — то есть по паре вопросов на человека за вечер,
# после чего сайт отвечал отказом всем сразу. Повторы бесплатны, они берутся
# из кэша, так что живой расход меньше числа нажатий.
RATE_LIMIT = 60
...
def _ask_yandex(prompt: str, image: tuple[bytes, str] | None = None) -> dict:
    ...
    # Формат, который OCR не принимает, — не поломка, но и не мелочь: читать
    # этикетку больше нечем, а картиночных моделей каталогу не открыли.
    # Раньше об этом узнавали из общего «не удалось», теперь говорим прямо:
    # из галереи такое приходит чаще, чем с камеры.
    if OCR_MIME.get(image[1]) is None:
        problems.append(f"этикетку в формате {image[1]} прочитать нечем")
...
def _photo_failure(problems: list[str]) -> str:
    ...
    if any("прочитать нечем" in problem for problem in problems):
        parts.append(
            ...
            "пришлите JPEG или PNG — например, переснимите камерой."
        )
