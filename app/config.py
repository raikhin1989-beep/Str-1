...
def photo_lookup() -> bool:
    ...
    отвечает 403, пока не сделаны обе разные вещи: сервисному аккаунту не
    выдана роль ai.vision.user на каталог, а API-ключу не задана область
    действия yc.ai.vision.execute. Роль даёт право аккаунту, область разрешает
    ключу этим правом пользоваться. Второго пути нет: моделей, понимающих
    картинку, каталогу не открыто (docs/PLAN.md, шаг 4).
