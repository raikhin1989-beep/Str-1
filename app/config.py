...
def fallback_base_url() -> str:
    """Запасной адрес сайта — по IP и порту, мимо домена.

    Нужен не «на всякий случай». Поймано на живой дегустации: у гостя с
    включённым VPN страница итогов не открылась вовсе. Домен у нас на
    duckdns.org, а этот суффикс целиком режут многие VPN и блокировщики
    рекламы — он давно на слуху у их чёрных списков. Сайту от этого плохо
    ровно там, где он нужен, и починить со своей стороны нечего: имя
    не резолвится ещё до нас.

    Запасной вход на 8081 от домена не зависит, поэтому по нему страница
    открывается и с VPN. Адрес задаётся деплоем из SERVER_HOST.
    """
    return os.environ.get("FALLBACK_BASE_URL", "").strip().rstrip("/")
